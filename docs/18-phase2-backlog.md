...
| A2 ✅ | web-staff PWA hoàn thiện | Workbox/offline cache, camera + OCR check-in, nối API đầy đủ | PROGRESS 6.6 | ✅ **Done** — _feature đã có sẵn từ task 6.6 (service worker precache + stale-while-revalidate API GET, luồng camera→OCR→check-in, đủ API hooks); mô tả cũ "mới shell tĩnh" là **stale**._ Bổ sung phần thực sự thiếu = **E2E Playwright web-staff** (profile điện thoại Pixel 5): smoke 2 vai (lễ tân→`/today`; buồng phòng→`/cleaning` + ẩn tab "Hôm nay") · luồng check-in (chạm khách đến → quét/nhập tay → **cổng consent NĐ13 chặn khi chưa đồng ý**) · **offline shell** (SW kiểm soát trang + fallback `/offline` khi mất mạng). Job CI `e2e-web-staff`; kèm fix SW đăng-ký-ngay khi trang đã `load` (tránh lỡ sự kiện → SW không bao giờ đăng ký). **5/5 xanh** local. **Còn:** camera OCR end-to-end verify trên thiết bị thật (cần camera) + MinIO/OCR provider (C4). | P1 |
...
| B2 🟡 | Notifications nâng cấp | Template MJML/Handlebars; **SMS/ZNS provider thật** (đang stub); e2e đầu-cuối qua queue | PROGRESS 4.4 TODO | 🟡 **Email template xong** — `EmailTemplateService` (Handlebars + layout HTML responsive, auto-escape biến) → kênh EMAIL gửi kèm HTML thương hiệu + text fallback (`mail.send` nhận `html`). Unit test (render + escaping) + e2e Mailpit assert HTML. **Còn:** SMS/ZNS provider thật (cần creds) + e2e đầu-cuối qua BullMQ queue (cần worker chạy); nguồn `.mjml` precompile nếu cần template phức tạp hơn. | P1 |
| B3 ✅ | Audit mở rộng | Ghi LOGIN/LOGOUT/EXPORT; archive partition cũ (>12 tháng) ra S3 | PROGRESS 4.5 TODO | ✅ **Done** — LOGIN/LOGOUT ghi tường minh trong `AuthService` (entity `auth`, kèm ip/user-agent); EXPORT qua decorator `@AuditExport` → `AuditInterceptor` ghi GET báo cáo (`after_data` = bộ lọc, không PII). e2e audit-log 11/11 (thêm LOGIN/LOGOUT/EXPORT). Archive partition: DETACH→`audit_logs_archived_YYYY_MM` đã có sẵn (0026/task 8.5); **dump S3 hoãn theo C4** (Ghi chú ↓) | P2 |
| B4 🟡 | Billing SaaS hoàn chỉnh | Cổng thanh toán tự động (thay confirm thủ công); **platform-auth module** (thay `PLATFORM_ADMIN_SECRET` header) | PROGRESS 4.7 TODO | 🟡 **Platform-auth xong** — module `platform-auth`: `POST /platform/auth/login` (bảng `platform_users`, JWT ký `JWT_PLATFORM_SECRET` + `typ:'platform'`, tách biệt token tenant) + `PlatformAuthGuard` bảo vệ confirm thuê bao (thay header `PLATFORM_ADMIN_SECRET` → deprecated). e2e platform-auth 8/8 + billing-lite cập nhật. **Còn:** cổng thanh toán tự động (auto-confirm thay thủ công) — cần creds gateway; 2FA/throttle platform-login (bảng đã có sẵn cột 2FA) | P1 |
...
| B6 ✅ | Police report Phase 2 | `police_report_status` trên booking + nối API dịch vụ công cư trú cấp xã (hiện manual export Excel) | PROGRESS 7.2 / docs/12 §2 | ✅ **Done** — cột `police_report_status` (PENDING/SUBMITTED/FAILED, `0028`) trên bookings; `POST /compliance/police-report/submit` chuyển khách đã lưu trú PENDING→SUBMITTED (idempotent: đã SUBMITTED→skip; thiếu số giấy tờ→FAILED) + audit STATE_CHANGE; lộ `police_report_status` trong BookingResponse. **API dịch vụ công cư trú (cấp xã) thật = STUB** (chưa có hợp đồng/endpoint; sau cải cách 2025 khai báo ở cấp xã, cần mã ĐVHC — NSO) — khi có thì thay nhánh stub bằng POST ngoài `withTenant`. e2e police-report 12/12 (+7 ca B6) | P2 |
...
| D2 ✅ | Tune & giám sát connection pool | Cấu hình `connection_limit`/pgbouncer; giám sát query-cost theo tenant (noisy neighbor) | [docs/02 §11–§12](02-multi-tenancy.md) · docs/11 §10 | ✅ **Done** — **Pool có giới hạn rõ**: `connection_limit`+`pool_timeout` tường minh trong `DATABASE_URL` (`.env.example`) + hướng dẫn PgBouncer transaction-mode (RLS-safe vì GUC LOCAL, ADR-0002 §4; Prisma `?pgbouncer=true`) + công thức sizing. **Giám sát cost theo tenant**: `withTenant` đo thời lượng mỗi unit-of-work → observer log `evt:'slow_tenant_tx'` {tenant_id, duration_ms, read_only, request_id, url} khi vượt `DB_SLOW_TX_LOG_MS` (env, mặc định 0=tắt → zero overhead); dashboard gom theo `tenant_id`. Runbook [docs/17 §9](17-oncall-runbook.md) (sizing + PgBouncer + pg_stat_activity bão hoà + cách bật). Verify: unit 4/4 + e2e rate-plans 9/9 (boot DI + observer bắn 59 log @1ms). _(Visual dashboard = bước ops trên log pipeline, như C1.)_ | P1 |
...
- **B3 Audit — dump S3 (hoãn theo C4):** action LOGIN/LOGOUT/EXPORT đã ghi `audit_logs` + e2e. Phần "archive ra S3": hàm SECURITY DEFINER `audit_archive_old_partitions()` (0026/task 8.5) đã DETACH partition >N tháng → bảng độc lập `audit_logs_archived_YYYY_MM` (KHÔNG drop). Việc **dump các bảng archived đó lên S3 rồi drop** phụ thuộc object storage prod → gộp vào **C4**; làm cùng lúc bật MinIO/S3 (presign/upload). Tới đó: cron gọi pg_dump từng `audit_logs_archived_*` → put S3 → `DROP TABLE`.
