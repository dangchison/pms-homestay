...
| B6 ✅ | Police report Phase 2 | `police_report_status` trên booking + nối API quận/huyện (hiện manual export Excel) | PROGRESS 7.2 / docs/12 §2 | ✅ **Done** — cột `police_report_status` (PENDING/SUBMITTED/FAILED, `0028`) trên bookings; `POST /compliance/police-report/submit` chuyển khách đã lưu trú PENDING→SUBMITTED (idempotent: đã SUBMITTED→skip; thiếu số giấy tờ→FAILED) + audit STATE_CHANGE; lộ `police_report_status` trong BookingResponse. **API quận/huyện thật = STUB** (chưa có hợp đồng/endpoint dịch vụ công) — khi có thì thay nhánh stub bằng POST ngoài `withTenant`. e2e police-report 12/12 (+7 ca B6) | P2 |
