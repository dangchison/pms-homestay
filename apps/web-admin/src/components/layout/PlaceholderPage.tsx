...
export function PlaceholderPage({
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
      <span className="flex size-16 items-center justify-center rounded-2xl bg-chip-brand-soft">
        <Icon className="size-7 text-chip-brand" />
      </span>
      <h1 className="mt-5 text-xl font-semibold tracking-tight">{title}</h1>
      <p className="mt-2 max-w-md text-sm text-muted-foreground">{description}</p>
      <span className="mt-3 rounded-full border border-border bg-surface px-3 py-1 text-xs text-muted-foreground">
        Theo lộ trình: {plan}
      </span>
