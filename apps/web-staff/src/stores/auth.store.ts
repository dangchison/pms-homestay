 ...
 * refresh token nằm trong cookie HTTP-only do BE set → mở lại app trong 30 ngày
 * không phải đăng nhập lại (ui/02 §PWA). `status` để AuthGate biết đã bootstrap
 * xong chưa (idle → đang thử refresh; authenticated/unauthenticated).
 */
