...
| **4** | 7–8 | Finance + Realtime + Audit | 🟡 **Finance ✅ (3.2–3.8 đóng EPIC 3)** · **Realtime + Notif ✅ (4.2 outbox+SSE + 4.3 emit + 4.4 notifications)**; còn 4.1 cleaning + Audit 4.5 |
...
- ⬜ 4.1 Cleaning · 4.5 Audit log · 4.7 Billing-lite SaaS
...
3. Kế tiếp **EPIC 4**: **4.1 Cleaning** (auto khi CHECKED_OUT/switch — đã có stub trong bookings; thêm migration `cleaning_tasks` + emit `cleaning_task.*` + đổi `housekeeping_status`) · **4.5 Audit log** (partition tháng + AuditInterceptor redact PII; READ_PII đã có path ở 2.5) · **4.7 Billing-lite SaaS**. Hoặc **EPIC 6 FE**: web-admin nối API + `useEvents` (SSE + 13 event + inbox `GET /notifications`) → realtime calendar (6.2)/booking form (6.3)/invoice-payment (6.4)/reports (6.5 — API 3.7 sẵn).
